...
$sorularıGetir = $db->query("SELECT * FROM questions")->fetchAll(PDO::FETCH_ASSOC);

// Soru silme işlemi
if (isset($_POST['soru_sil']) && !empty($_POST['soru_id'])) {
    $soru_id = $_POST['soru_id'];
    $soruSilmeSorgusu = $db->prepare("DELETE FROM questions WHERE id = ?");
...
if (isset($_POST['soru_ekle'])) {
    ...
    $soruEklemeSorgusu = $db->prepare("INSERT INTO questions (question, option1, option2, option3, option4, correct, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['soru_duzenle']) && !empty($_POST['soru_id'])) {
    ...
    $correct = $_POST['doğru'];
    $difficulty = $_POST['difficulty'];
    $soruDuzenlemeSorgusu = $db->prepare("UPDATE questions SET question = ?, option1 = ?, option2 = ?, option3 = ?, option4 = ?, correct = ?, difficulty = ? WHERE id = ?");
    $soruDuzenlemeSorgusu->execute([$question, $option1, $option2, $option3, $option4, $correct, $difficulty, $soru_id]);
    $sorularıGetir = $db->query("SELECT * FROM questions")->fetchAll(PDO::FETCH_ASSOC);
}

// Soru düzenleme için seçilen soruyu getir
$duzenlemeSoruId = null;
$duzenlemeSoru = null;
if (isset($_GET['edit']) && !empty($_GET['edit'])) {
    $duzenlemeSoruId = $_GET['edit'];
    $duzenlemeSoru = $db->prepare("SELECT * FROM questions WHERE id = ?");
...
<body>
    <div class="container">
        <a href="quiz.php"><button class="large-button">Yarışmaya Başla</button></a>
        ...
        if (!empty($sorularıGetir)) {
            foreach ($sorularıGetir as $soru) {
                ...
                echo '<a href="admin.php?edit=' . htmlspecialchars($soru['id']) . '">Düzenle</a>';
                
                // Soru Silme
                echo '<form method="POST" action="admin.php" style="display:inline; margin-left: 10px;">';
                echo '<input type="hidden" name="soru_id" value="' . htmlspecialchars($soru['id']) . '">';
        ...
        <?php if ($duzenlemeSoru): ?>
            ...
            <form method="POST" action="admin.php">
                <input type="hidden" name="soru_id" value="<?php echo htmlspecialchars($duzenlemeSoru['id']); ?>">
