...
// Sepete ekleme işlemi için AJAX çağrısını işlemek
...
// Popüler yemekleri veritabanından çekme
...
<head>
    ...
    <link rel="stylesheet" href="css/styles.css"> <!-- CSS dosyasını ekleyin -->
    <script>
        function addToBasket(foodId) {
            const formData = new FormData();
            formData.append('food_id', foodId);

            fetch('famous.php', {  // Bu dosyaya gönderim yapıyoruz
                method: 'POST',
                body: formData
            })
            .then(response => response.text())
            .then(data => {
                alert(data); // İşlem sonucu kullanıcıya gösteriliyor
            });
        }
    </script>
</head>
